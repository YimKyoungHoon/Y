...
배열이 가지고있는 값을 요소라고 합니다
이 요소에는 값으로 인정하는 모든것이 올수있습니다
let sum = [1, 2, 3, 4, 4+3,function(){}]; 
표현식, 함수등도 값으로 인정되기 때문에 요소가 될수있습니다

자바스크립트 배열의 특징
일반 자료의 구조에서 말하는 배열은 밀집배열이라고 합니다
밀집배열은 동일한 데이터 크기가 빈틈없이 
연속적으로 이어져 있는 구조입니다
자바스크립트 배열은 각 메모리공간이 동일한 크기가 아니고,
연속적으로 메모리 공간이 이어져있지도 않습니다
또 희소배열을 포함하기도 합니다.
결론적으로 js배열은 일반적인(자료구조의) 배열을 흉내낸 특수객체이다
즉 인덱스라는 문자열을 프로퍼티의 이름(key)을 가지고
해당요소의 값을 이어서 가지는 특수 객체입니다

희소배열이란?
빈 값이 있는 배열을 뜻합니다
할당되지 않는 인덱스에 초기값 undefined를 넣은 배열, 
이배열은 반복문을 돌릴때 문제가 됩니다, 비어있는 값은 반복을 못함
...
//배열리터럴로 생성하는 방법
const arr = [1, 2, 3];
//빈배열을 선언하여 만들고, 배열의 값을 이후에 하나씩 넣는방법
const arr1 = [];
arr[0] = 1;
arr[1] = 2;
...
대괄호안에는 인덱스를 씁니다
정수로 평가되는 표현식이라면 인덱스 위치에 올수있습니다
만약 정수값이 아닌경우, 오류는 나지 않습니다만
이렇게되면 요소가 생성된것이 아니라 프로퍼티가 생성됩니다
const arr = [];
arr[0] = 1;
arr["1"] = 2;
arr[1.4] = 3;
arr["apple"] = "핸드폰";
//1.4라는 프로퍼티 이름과, 값 3으로 들어간것이고, 배열의 요소는 아님
보통은 정수가 아닌값을 넣지않습니다
하지만 실수로 정수가 아닌 실수를 넣을경우 오류가 나지 않아
개발자가 알수없으므로 주의요함
예) 5개의 요소를 넣기로 했는데 실수로 마지막요소를 정수가아닌
실수값을 넣으면, 오류가 나지않아서 개발자는 깨닫지 못하고
이상태에서 반복문이나 기타 배열관련 메소드를 사용하면
원하는 값을 얻지 못하게됩니다
...
//두번째 값으로 인덱스(순번)을 적으면, 검색을 시작할 인덱스를 지정
//따라서 2인덱스부터 2요소를 찾고 그 찾은 인덱스를 반환하게 됩니다
//따라서만약 두번째 값인 시작할 인덱스를 생략하면 처음부터 찾는것입니다

2. push
const arr = [1, 2, 2, 3];
console.log(arr.push(3, 4));
//push에 넣은 값을 원본배열 arr의 마지막요소에 추가하고
//변경된 arr.length를 반환합니다
console.log(arr);
//이 메소드는 원본배열을 직접 변경합니다
//만약 push로 추가할 요소가 하나라면 다른방법으로 추가할수 있습니다
arr[arr.length] = 3;
//arr이라는 배열의 길이를 인덱스로 넣어서 가장마지막이라는 뜻으로
//사용하여 push가 가진뜻 가장 마지막인덱스에 추가한다는 뜻을
//다른방법(배열을 참조하는방법)으로 충족한것
3. pop
const arr = [1, 2, 2, 3];
console.log(arr.pop());
//배열 arr의 가장 마지막요소인 3을 제거하고 제거한 요소인
//3을 반환합니다
console.log(arr);
//원본배열을 직접 변경합니다
4. unshift
...
//배열 arr의 첫번째 요소를 제거하고, 제거한 요소를 반환합니다
console.log(arr);
//원본 배열을 직접 변경합니다
...
//추가되는 값으로 배열도 올수있지만 원시값도 넣을수있습니다
console.log(arr1);
console.log(arr2);
//원본 배열은 변경되지 않습니다
...
let arr3 = [1, 2].concat(arr2);
arr3 = arr3.concat(5, 6);
//const상태에서는 원시형자료형태로 5,6을 넣게되면
//const상태이기 때문에 타입에러가 발생합니다
//따라서 let으로 선언을 하거나, 혹은 (5,6)을 원시형이 아닌
//배열을 만든뒤 참조형 자료로 추가하면 해결됩니다
console.log(arr3);
...
// 배열.splice
//(시작할인덱스,몇개를 제거할지의 갯수,(제거한 자리에)추가할요소들);
...
//2인덱스로 시작해서 뒤에모두를 제거합니다
//따라서 원본배열은 [1,2]만남게되고
//반환 배열은 삭제한요소 [2,3]을 반환합니다

// const res = arr.splice(-1);
//시작할 인덱스에 음수를 적는게 가능한데 이때 -1을 쓰게되면
//원본배열의 가장 마지막요소를 가르키는것
// -n은 마지막에서 n번째를 가리키는것
//요소가 많아서 뒤에서부터 카운팅이 더 효율적인경우 사용합니다
// console.log(arr);
// console.log(res);

//몇개를 제거할지의 갯수
//옵션: 즉 생략이 가능합니다
//0인경우 아무요소도 제거하지 않습니다
...
//-n하면 마지막에서 n번째요소, 요소가 많을경우 사용합니다
...
10. reverse
원본배열의 순서를 반대로 뒤집으며, 뒤집은 배열을 반환합니다
원본배열이 변경됩니다
const arr = [1, 2, 2, 3];
arr.reverse();
...
// arr.fill("a"); // ['a', 'a', 'a', 'a']
// arr.fill("a", 1); // [1, 'a', 'a', 'a']
//두번째 값을 넣으면 요소 채우기를 시작할 인덱스를 쓸수있습니다
arr.fill("a", 1, 2);
//세번째 값을 넣으면 요소 채우기를 멈출 인덱스를 쓸수있습니다
console.log(arr);

//fill메서드를 사용해서 배열 생성하고, 이후 특정값으로 요소를 모두
//채울수 있습니다
const creArr = new Array(4); //[empty × 4]
console.log(creArr);
// // [] //배열의 길이가 정해지지 않았음 
// // [,,,] //배열의 길이가 4칸으로 정해진상태
...
//indexOf메서드였습니다. 이때는 특정 인덱스 정수의 값을 반환하거나
//없는 경우 -1을 반환했습니다
//반면 includes 는 불린값을 반환합니다
//NaN (not a number)
//만약 해당배열에 NaN이 포함되어있으면 includes는 배열안의 요소
//인식하여 찾을수 있으나
//indexOf는 NaN을 요소로 인식을 하지못해서 찾을수 없습니다
...
//실습
//includes 로 변경해보세요~
//indexOf ===-1값이라는것은 없다는 부정의의미입니다
//따라서 기본 긍정의 의미인 includes와 같으려면
//부정을 해주어야합니다 따라서 ! 부정의 논리연산자를 사용
...
향상시키며, 버그를 줄일 수 있어서 안전한 코딩을 할수있게 도와줍니다

=== 자바스크립트에서 불변성??
1. 예측가능성 : 예상치 못한 부작용을 줄일수 있음
2. 디버깅과 유지보수 : 원본배열이 있다는 것은, 버그가 나거나 유지보수
중에 반환배열(변경된)의 비교를 통해서 버그를 찾아내거나, 실수를 찾거나
업데이트(유지보수)를 함에있어 큰 도움을 줍니다
3. 함수형프로그래밍 : 자바스크립트는 함수형 프로그래밍을 지향하며
이는 상태변경을 피하고, 데이터 변환을 함수의 조합으로 표현하고자하는
프로그래밍 철학입니다.따라서 자바스크립트는 불변성을 지켜서 
함수형 프로그래밍을 하려고노력합니다
4. 성능최적화 : 이중( 원본배열, 반환배열 )으로 병행작업을 
할경우 좀더 작업이 쉽고 성을 최적화 시킬 수 있습니다
5. react(라이브러리) : 불변성이 중요한 라이브러리와 
효율적인 통합관리로서의 필요성이 있습니다
리얼돔 : document를 직접 구현한 DOM을 의미
가상돔 : document에는 없을수있는 js에서 앞으로(미래시)
생성될거라 예상하는 시나리오의 DOM의 모습

배열의 고차함수
1. sort
배열의 요소를 정렬하며, 원본배열을 직접변경하고 정렬된 배열을 반환
기본적으로 오름차순으로 정렬합니다
...
//내림차순으로 하고싶다면, 먼저 sort로 오름차순 정렬이후
//reverse를 이용합니다
const res2 = foods.reverse();
console.log(res2); //['orange', 'banana', 'apple']
//여기까지 문자에 관련한 sort입니다
//문자의 오름차순 정렬은 문제없지만 숫자요소는 문제가 있다
...
//그런데 2는 U+0032
//이런이유로 숫자 정렬은 비교함수를 생성해서 정렬순서를 정의해주어야함
//비교함수 적용 sort코드
...
6. findIndex
말그래도 find는 요소를 반환했다면, 이것은 인덱스를 반환합니다
즉 배열의 요소를 순회하면서 콜백함수의 true값을 반환하는 
해당 요소의 인덱스를 반환합니다
해당요소의 값이없으면 -1을 반환합니다
const point = [40, 100, 1, 5, 2, 25, 10];
